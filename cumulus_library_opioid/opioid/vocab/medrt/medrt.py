...
RXNORM_SAB = ['ATC', 'CVX', 'DRUGBANK', 'GS',
              'MMSL', 'MMX', 'MTHCMSFRF' 'MTHSPL',
...
def curated_cui_root() -> str:
    _sql = [create('curated_cui'),
            "select  distinct 'CUI1' as tier, CUI1  as CUI, R.*",
            f"from    MRREL_medrt_cui1 R", ";",
            index('curated_cui', 'CUI'),
            index('curated_cui', 'SAB,CODE')]
    return '\n'.join(_sql)

def curated_cui(tier) -> str:
    _sql = ["insert into curated_cui"
            f" select distinct 'CUI{tier}' as tier, CUI{tier} as CUI, R.* ",
            f" from MRREL_medrt_cui{tier} as R", ";"]
    return '\n'.join(_sql)

def MRREL_medrt() -> str:
    tablename = f"MRREL_medrt"
    _sql = [create(tablename),
            f" select * from umls.MRREL ",
            f" WHERE REL = 'CHD' ",
            f" OR   RELA in ({tic(MEDRT_RELA)})", ";",
            f" delete from MRREL_medrt where REL in ('RB', 'PAR')", ";",
            index(tablename, 'CUI1'),
            index(tablename, 'SAB')]
    return '\n'.join(_sql)

def MRREL_medrt_cui_root() -> str:
    tablename = f"MRREL_medrt_cui1"

    _select = 'select  distinct R.CUI1, R.REL, R.RELA, R.CUI2, R.SAB, C.TTY, C.CODE, C.STR'
...
def MRREL_medrt_cui(tier: int) -> str:
    ...
    _sql = [create(tablename),
            ...
            f" D.SAB, D.TTY, D.CODE, D.STR ",
            f" FROM "
            f" MRREL_medrt_cui{tier-1} as I,",
            f" MRREL_medrt  as R,",
            f" MRCONSO_drug as D ",
...
def medrt_non():
    return medrt(str_like(['Benzodiazepine',
                           'Barbiturate',
