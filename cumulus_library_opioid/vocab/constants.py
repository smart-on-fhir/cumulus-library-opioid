...
def jaccard_intersect_size() -> str:
    return (
        "INSERT into jaccard_intersect_size  "
...
def jaccard_difference_size() -> str:
    return (
        "INSERT into jaccard_difference_size  "
...
def make_recursive(vsac) -> str:
    ...
    _make = ["rm -f curated.tsv", f"ln -s infile/${vsac}.tsv curated.tsv", "./make.sh"]
...
def link_tier(tier="curated") -> str:
    ...
    for vsac in CURATED_LIST:
        _cmd += [f"rm -f {vsac}.tsv", f"ln -s {vsac}/{vsac}.{tier}.tsv {vsac}.tsv\n"]
    return "\n".join(_cmd)
...
class JaccardTest(unittest.TestCase):
    def test_jaccard(self):
        ...
        with open("jaccard_insert.sql", "w") as fp:
